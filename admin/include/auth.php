...
class auth extends database{
    ...
    public function registeruser($name, $username, $email, $address, $city, $country, $phone, $password)
    {
    ...
    public function update_pass($password,$username)
    {
        ...
        public function delteCartData($delete_cart){
          $sql = "DELETE FROM cart WHERE product_id=:delete_cart"; 
          $stmt=$this->conn->prepare($sql);
          $stmt->bindParam(':delete_cart' ,$delete_cart ,PDO::PARAM_STR);
          $stmt->execute();
          return true ;
    ...
    public function fetchByCategory($cat_id){
      $sql ="SELECT category.*, products.id,products.p_name,products.p_price,products.p_discount,products.images from category inner join products on products.category_id = category.id where category.id = $cat_id";
      $stmt = $this->conn->prepare($sql);
      $stmt->execute();
      $result=$stmt->fetchAll(PDO::FETCH_ASSOC);
      return $result;
    ...
    // Update Sub Category To Database
     ...
     //  SELECT PRoDUCTS from database 
      ...
      public function select_products_by_cat($edit_id){
        $sql = "SELECT products.*, category.id,category.cat_name FROM products inner join category on products.category_id = category.id WHERE products.id = $edit_id";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute();
        $result=$stmt->fetch(PDO::FETCH_ASSOC);
        return $result;
    ...
    return true ;
      ...
      public function delete_notification($n_id)
      {
        ...
        return true ;
                   ...
                   public function products_last_three(){
                  ...
                  public function search_form($search)
                  {
                    $sql = "SELECT * FROM products WHERE p_name LIKE '%".$search."%' OR p_price LIKE '%".$search."%'";
                    $stmt =$this->conn->prepare($sql);
